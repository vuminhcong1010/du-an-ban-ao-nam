        ...
        <div class="personal-info-column">
          ...
          <div class="input-group gender-selection"> <label>Giới tính</label>
            <div class="gender-options">
              ...
              <label class="radio-label">
                <input type="radio" v-model="form.gioiTinh" :value="null" name="gender" class="radio-input"> Không xác
                định
              </label>
            </div>
          </div>
...
const errors = ref({
  ...
  diaChiXa: false,
  diaChiChiTiet: false,
  emailExists: false,       // Cờ lỗi trùng email (cho FE check)
  soDienThoaiExists: false, // Cờ lỗi trùng số điện thoại (cho FE check)
});
...
// --- Watchers cho kiểm tra trùng lặp email/sdt (frontend) ---
watch(() => form.value.email, async (newEmail) => {
  errors.value.emailExists = false;
  if (newEmail && validateEmail(newEmail)) {
    try {
      const response = await apiClient.get(`/api/khach-hang/check-email?email=${newEmail}`);
...
watch(() => form.value.soDienThoai, async (newPhone) => {
  ...
  if (newPhone && validatePhone(newPhone)) {
    try {
      const response = await apiClient.get(`/api/khach-hang/check-phone?phone=${newPhone}`);
      if (response.data.exists) {
...
const handleSubmit = async () => {
  ...
  if (!form.value.email) {
    ...
    errors.value.email = true; // Set lỗi định dạng
    formIsValid = false;
  } else {
    // Chỉ kiểm tra trùng lặp qua API nếu định dạng email hợp lệ
    try {
      const response = await apiClient.get(`/api/khach-hang/check-email?email=${form.value.email}`);
      if (response.data.exists) {
        errors.value.emailExists = true;
        formIsValid = false;
      }
      ...
      // Không ngăn form submit nếu chỉ là lỗi kết nối check trùng lặp, để BE xử lý cuối cùng
    }
  }
  ...
  if (!form.value.soDienThoai) {
    ...
    errors.value.soDienThoai = true; // Set lỗi định dạng
    formIsValid = false;
  } else {
    // Chỉ kiểm tra trùng lặp qua API nếu định dạng SĐT hợp lệ
    try {
      const response = await apiClient.get(`/api/khach-hang/check-phone?phone=${form.value.soDienThoai}`);
      if (response.data.exists) {
        errors.value.soDienThoaiExists = true;
        formIsValid = false;
      }
  ...
  // VALIDATION FOR ADDRESS (giữ nguyên logic của bạn)
  ...
  try {
    const payload = {
      ...
      // Không gửi tenTaiKhoan và matKhau
    };
    ...
    if (err.response) {
      ...
      if (status === 400) { // Bad Request
        if (typeof errorData === 'string') {
          // Lỗi từ IllegalArgumentException hoặc DataIntegrityViolationException (String body)
          errorMessage = errorData;
          // Cố gắng ánh xạ lỗi cụ thể nếu message chứa từ khóa
          if (errorData.includes("Email đã tồn tại")) errors.value.emailExists = true;
          if (errorData.includes("Số điện thoại đã tồn tại")) errors.value.soDienThoaiExists = true;
        } else if (typeof errorData === 'object' && errorData !== null) {
          if (errorData.errors && Array.isArray(errorData.errors)) { // Từ @Valid (Spring Boot > 2.x thường trả về thế này)
            errorMessage = "Thông tin không hợp lệ:\n" + errorData.errors.map(e => e.defaultMessage || e.field).join(";\n");
            errorData.errors.forEach(e => {
              if (e.field) {
                if (e.field === 'tenKhachHang') errors.value.tenKhachHang = true;
                else if (e.field === 'email') errors.value.email = true;
                else if (e.field === 'soDienThoai') errors.value.soDienThoai = true;
                else if (e.field === 'ngaySinh') errors.value.ngaySinh = true;
                else if (e.field.startsWith('diaChi.')) {
                    const addressField = e.field.split('.')[1];
                    if (addressField === 'tinhThanhPho') errors.value.diaChiTinh = true;
                    else if (addressField === 'quanHuyen') errors.value.diaChiQuan = true;
                    else if (addressField === 'xaPhuong') errors.value.diaChiXa = true;
                    else if (addressField === 'diaChiChiTiet') errors.value.diaChiChiTiet = true;
                }
              }
            });
          } else if (Object.keys(errorData).length > 0) { // Nếu BE trả về map trực tiếp field -> message
            const errorMessages = Object.entries(errorData)
              .map(([field, msg]) => {
                if (field === 'tenKhachHang') { errors.value.tenKhachHang = true; return `Tên khách hàng: ${msg}`; }
                ...
                if (field === 'diaChiTinh') { errors.value.diaChiTinh = true; return `Tỉnh/Thành phố: ${msg}`; }
                if (field === 'diaChiQuan') { errors.value.diaChiQuan = true; return `Quận/Huyện: ${msg}`; }
                if (field === 'diaChiXa') { errors.value.diaChiXa = true; return `Xã/Phường: ${msg}`; }
                if (field === 'diaChiChiTiet') { errors.value.diaChiChiTiet = true; return `Địa chỉ chi tiết: ${msg}`; }
                // Xử lý các lỗi trùng lặp từ BE
                if (msg.includes("Email đã tồn tại")) { errors.value.emailExists = true; return msg; }
                if (msg.includes("Số điện thoại đã tồn tại")) { errors.value.soDienThoaiExists = true; return msg; }
                return `${field}: ${msg}`;
              })
              .join(";\n");
            errorMessage = "Thông tin không hợp lệ:\n" + errorMessages;
      ...
      } else { // Các lỗi HTTP khác ngoài 400
