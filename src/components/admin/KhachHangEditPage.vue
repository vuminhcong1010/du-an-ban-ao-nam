...
const errors = ref({
  ...
  soDienThoai: false,
  ngaySinh: false,
  emailExists: false,       // Cờ lỗi trùng email
  soDienThoaiExists: false, // Cờ lỗi trùng số điện thoại
});
...
const form = ref({
  ...
  soHoaDonDaMua: null,
  tongTien: null,
  originalEmail: '',       // Để lưu email gốc khi tải
  originalSoDienThoai: '', // Để lưu SĐT gốc khi tải
});
...
// --- Watchers cho kiểm tra trùng lặp email/sdt (frontend) khi update ---
watch(() => form.value.email, async (newEmail) => {
  errors.value.emailExists = false;
  // Chỉ kiểm tra nếu email không rỗng, định dạng hợp lệ và khác với email ban đầu của khách hàng này
  if (newEmail && newEmail !== form.value.originalEmail && validateEmail(newEmail)) {
    try {
      // Truyền ID của khách hàng để backend loại trừ email của chính khách hàng này
      const response = await apiClient.get(`/api/khach-hang/check-email?email=${newEmail}&id=${form.value.id}`);
      if (response.data.exists) {
        errors.value.emailExists = true;
      }
    } catch (err) {
...
watch(() => form.value.soDienThoai, async (newPhone) => {
  errors.value.soDienThoaiExists = false;
  // Chỉ kiểm tra nếu số điện thoại không rỗng, định dạng hợp lệ và khác với số điện thoại ban đầu
  if (newPhone && newPhone !== form.value.originalSoDienThoai && validatePhone(newPhone)) {
    try {
      // Truyền ID của khách hàng để backend loại trừ SĐT của chính khách hàng này
      const response = await apiClient.get(`/api/khach-hang/check-phone?phone=${newPhone}&id=${form.value.id}`);
      if (response.data.exists) {
        errors.value.soDienThoaiExists = true;
      }
    } catch (err) {
...
const fetchCustomerDetails = async (id) => {
  try {
    const response = await apiClient.get(`/api/khach-hang/${id}`);
    const customerData = response.data;

    form.value.maKhachHang = customerData.maKhachHang ?? '';
    ...
    form.value.gioiTinh = customerData.gioiTinh === true;
    form.value.ngaySinh = customerData.ngaySinh?.split('T')[0] ?? '';
    form.value.trangThai = customerData.trangThai ?? 1;
    form.value.soHoaDonDaMua = customerData.soHoaDonDaMua ?? 0;
    form.value.tongTien = customerData.tongTien ?? 0;

    // LƯU CÁC GIÁ TRỊ GỐC ĐỂ SO SÁNH TRONG WATCHER (RẤT QUAN TRỌNG ĐỂ PHÂN BIỆT ĐỔI VỚI CHÍNH MÌNH VÀ ĐỔI VỚI NGƯỜI KHÁC)
    form.value.originalEmail = customerData.email ?? '';
    form.value.originalSoDienThoai = customerData.soDienThoai ?? '';
...
const fetchAddresses = async () => {
  if (form.value.id) {
    try {
      const response = await apiClient.get(`/api/khach-hang/${form.value.id}/dia-chi`);
...
const handleSubmit = async () => {
  ...
  if (!form.value.tenKhachHang || !isFullName(form.value.tenKhachHang)) {
    // Nếu bạn có cờ lỗi riêng cho tenKhachHang trong errors ref, hãy uncomment dòng dưới
  ...
  // VALIDATION EMAIL
  if (!form.value.email) {
    // errors.value.email = true; // Chỉ cần set nếu có cờ này trong errors ref
    formIsValid = false;
  } else if (!validateEmail(form.value.email)) {
    errors.value.email = true;
    formIsValid = false;
  } else if (form.value.email !== form.value.originalEmail) { // Chỉ check trùng lặp nếu email thay đổi
    try {
      const response = await apiClient.get(`/api/khach-hang/check-email?email=${form.value.email}&id=${form.value.id}`);
      if (response.data.exists) {
        errors.value.emailExists = true;
        formIsValid = false;
      }
    } catch (err) {
      console.error("Lỗi khi kiểm tra trùng email (handleSubmit):", err);
    }
  }

  // VALIDATION SỐ ĐIỆN THOẠI
  if (!form.value.soDienThoai) {
    // errors.value.soDienThoai = true; // Chỉ cần set nếu có cờ này trong errors ref
    formIsValid = false;
  } else if (!validatePhone(form.value.soDienThoai)) {
    errors.value.soDienThoai = true;
    formIsValid = false;
  } else if (form.value.soDienThoai !== form.value.originalSoDienThoai) { // Chỉ check trùng lặp nếu SĐT thay đổi
    try {
      const response = await apiClient.get(`/api/khach-hang/check-phone?phone=${form.value.soDienThoai}&id=${form.value.id}`);
      if (response.data.exists) {
        errors.value.soDienThoaiExists = true;
        formIsValid = false;
      }
    } catch (err) {
      console.error("Lỗi khi kiểm tra trùng số điện thoại (handleSubmit):", err);
    }
  }
  ...
  try {
    ...
    await apiClient.put(`/api/khach-hang/${form.value.id}`, customerUpdatePayload);
    ...
    if (err.response) {
      ...
      if (status === 400) { // Bad Request
        if (typeof errorData === 'string') {
          ...
          // Cố gắng ánh xạ lỗi cụ thể nếu message chứa từ khóa
          if (errorData.includes("Email đã tồn tại") && errorData.includes("khách hàng khác")) errors.value.emailExists = true;
          if (errorData.includes("Số điện thoại đã tồn tại") && errorData.includes("khách hàng khác")) errors.value.soDienThoaiExists = true;
        } else if (typeof errorData === 'object' && errorData !== null) {
          if (errorData.errors && Array.isArray(errorData.errors)) {
            errorMessage = "Thông tin không hợp lệ:\n" + errorData.errors.map(e => e.defaultMessage || e.field).join(";\n");
            errorData.errors.forEach(e => {
              if (e.field) {
                // Giả định field names từ BE khớp với keys trong errors ref
                if (e.field === 'email') errors.value.email = true;
                else if (e.field === 'soDienThoai') errors.value.soDienThoai = true;
                else if (e.field === 'ngaySinh') errors.value.ngaySinh = true;
                // Nếu có các trường khác cần validate (ví dụ: tenKhachHang), thêm vào đây
              }
            });
          } else if (Object.keys(errorData).length > 0) {
            const errorMessages = Object.entries(errorData)
              .map(([field, msg]) => {
                // Ánh xạ lỗi và đặt cờ
                if (field === 'email') { errors.value.email = true; return `Email: ${msg}`; }
                if (field === 'soDienThoai') { errors.value.soDienThoai = true; return `Số điện thoại: ${msg}`; }
                if (field === 'ngaySinh') { errors.value.ngaySinh = true; return `Ngày sinh: ${msg}`; }
                // Xử lý các lỗi trùng lặp từ BE
                if (msg.includes("Email đã tồn tại") && msg.includes("khách hàng khác")) { errors.value.emailExists = true; return msg; }
                if (msg.includes("Số điện thoại đã tồn tại") && msg.includes("khách hàng khác")) { errors.value.soDienThoaiExists = true; return msg; }
                return `${field}: ${msg}`;
              })
              .join(";\n");
            errorMessage = "Thông tin không hợp lệ:\n" + errorMessages;
          } else {
            errorMessage = "Lỗi từ máy chủ: " + (errorData.message || JSON.stringify(errorData));
          }
        }
      } else {
...
const editAddressInPopup = (address) => {
  selectedAddressForEdit.value = {
    id: address.id,
    tinhId: address.tinhThanhPho,
    quanId: address.quanHuyen,
    xaId: address.xaPhuong,
...
const deleteAddress = async (addressId, isMacDinh) => {
  ...
  if (result.isConfirmed) {
    try {
      await apiClient.delete(`/api/dia-chi/${addressId}`);
  ...
  } else if (result.dismiss === proxy.$swal.DismissReason.cancel) {
...
const setDefaultAddress = async (selectedAddress) => {
  try {
    await apiClient.put(`/api/dia-chi/${selectedAddress.id}/set-default/${form.value.id}`);
