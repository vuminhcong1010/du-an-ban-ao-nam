...
const updateSearchResult = () => {
  const filtered = res.value.filter(item =>
    (item.maTayAo || "").toLowerCase().includes(searchText.value.toLowerCase()) ||
    (item.tenTayAo || "").toLowerCase().includes(searchText.value.toLowerCase())
  );
...
const add = async () => {
  ...
  try {
    await axios.post("http://localhost:8080/tay-ao/add", req.value, {
      headers: { Authorization: `Bearer ${token}` }
    });
...
const update = async () => {
  ...
  const isDuplicate = res.value.some(
    (item) => item.tenTayAo.trim().toLowerCase() === req.value.tenTayAo.trim().toLowerCase() && item.id !== req.value.id
  );
  if (isDuplicate) {
    toast.error("Tên tay áo đã tồn tại!");
    return;
  ...
  try {
    await axios.post("http://localhost:8080/tay-ao/update", req.value, {
      headers: { Authorization: `Bearer ${token}` }
    });
...
const remove = async (id) => {
  try {
    await axios.get(`http://localhost:8080/tay-ao/delete/${id}`, {
      headers: { Authorization: `Bearer ${token}` }
    });
...
const detail = async (id) => {
  try {
    const response = await axios.get(`http://localhost:8080/tay-ao/detail/${id}`, {
      headers: { Authorization: `Bearer ${token}` }
    });
...
<template>
  <div class="mt-4 px-3">
    <div class="d-flex align-items-center justify-content-between bg-white p-3 rounded mb-3 border">
      <h2 class="fw-bold mb-0">Quản lý tay áo</h2>
      <div class="d-flex gap-2">
        <button @click="reset" data-bs-toggle="modal" data-bs-target="#exampleModal"
          ...
          <Plus class="me-1" size="16" /> Thêm mới
        </button>
      </div>
    </div>
    ...
    <div class="bg-white p-3 rounded mb-3 border">
      <h5 class="text-start mb-3">
        <FilterIcon size="16" class="me-1"></FilterIcon> Bộ lọc
      </h5>
      <input type="text" class="form-control" placeholder="Tìm theo mã, tên tay áo..." v-model="searchText" />
    </div>

    <div class="bg-white p-3 rounded mb-4 border">
      <h5 class="text-start ps-3 pt-3">Danh sách tay áo</h5>
