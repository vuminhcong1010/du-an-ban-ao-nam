...
const add = () =>{
    axios.post("http://localhost:8080/kieu-ao/add",req.value).then(Response =>{
      res.value = Response.data.data
      page.value = Math.ceil(Response.data.size/10)
      reset()
    }).catch (err =>{
...
const update = () =>{
    ...
    axios.post("http://localhost:8080/kieu-ao/update",req.value).then(Response =>{
      res.value = Response.data.data
      page.value = Math.ceil(Response.data.size/10)
      reset()
    }).catch (err =>{
...
const remove = (id) =>{
    axios.get(`http://localhost:8080/kieu-ao/delete/${id}`).then(Response =>{
      res.value = Response.data.data
      page.value = Math.ceil(Response.data.size/10)
    }).catch (err =>{
       console.log(err);
...
const detail = (id) =>{
    ...
    axios.get(`http://localhost:8080/kieu-ao/detail/${id}`).then(Response =>{
...
const paging = (id) =>{
    axios.get(`http://localhost:8080/kieu-ao/${id}`).then(Response =>{
      res.value = Response.data.data
      page.value = Math.ceil(Response.data.size/10)
    })
}
...
<template>
  <div class="container my-4">
    ...
    <div class="bg-white p-3 rounded shadow mb-4">
      ...
      <div class="bg-white p-3 rounded shadow mb-4">
      <div class="d-flex justify-content-between align-items-center mb-3">
        <h5 class="fw-semibold m-0">Quản lý kiểu áo</h5>
