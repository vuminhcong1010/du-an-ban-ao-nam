...
watch(searchText, (newValue) => {
  ...
  searchResult.value = res.value.filter(item =>
    (item.maKieuAo || "").toLowerCase().includes(newValue.toLowerCase()) ||
    (item.tenKieuAo || "").toLowerCase().includes(newValue.toLowerCase())
  );
});
...
const update = () => {
  ...
  const isDuplicate = res.value.some(
    (item) =>
      item.tenKieuAo.trim().toLowerCase() === req.value.tenKieuAo.trim().toLowerCase() &&
...
const save = (id) => {
  ...
  if (item) {
    req.value.tenKieuAo = item.tenKieuAo;
  }
};
        ...
        <div class="col-12">
          ...
          <div class="d-flex align-items-center gap-2">
            <input
              type="text"
              class="form-control"
              placeholder="Tìm theo mã, tên kiểu áo"
              v-model="searchText"
            />
          </div>
        </div>
