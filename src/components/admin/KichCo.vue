...
const updateSearchResult = () => {
  const filtered = res.value.filter(item =>
    (item.maKichCo || "").toLowerCase().includes(searchText.value.toLowerCase()) ||
    (item.soCo || "").toLowerCase().includes(searchText.value.toLowerCase())
  );
...
const add = async () => {
  if (!req.value.soCo.trim()) {
    toast.error("Vui lòng nhập đầy đủ thông tin");
    return;
  }
  const isDuplicate = res.value.some(
    (item) => item.soCo.trim().toLowerCase() === req.value.soCo.trim().toLowerCase()
  );
  if (isDuplicate) {
    toast.error("Tên kích cỡ đã tồn tại!");
    return;
  }
  const kichCoCoSan = ['S', 'M', 'L', 'XL', 'XXL', 'XXXL'];
  if (!kichCoCoSan.includes(req.value.soCo)) {
    toast.error("Kích cỡ không hợp lệ. Vui lòng chọn một trong các kích cỡ: S, M, L, XL, XXL, XXXL");
    return;
  }
  try {
    await axios.post("http://localhost:8080/kich-co/add", req.value, {
      headers: { Authorization: `Bearer ${token}` }
    });
...
const update = async () => {
  ...
  const isDuplicate = res.value.some(
    (item) => item.soCo.trim().toLowerCase() === req.value.soCo.trim().toLowerCase() && item.id !== req.value.id
  );
  if (isDuplicate) {
    toast.error("Tên kích cỡ đã tồn tại!");
    return;
  ...
  try {
    await axios.post("http://localhost:8080/kich-co/update", req.value, {
      headers: { Authorization: `Bearer ${token}` }
    });
...
const remove = async (id) => {
  try {
    await axios.get(`http://localhost:8080/kich-co/delete/${id}`, {
      headers: { Authorization: `Bearer ${token}` }
    });
...
const detail = async (id) => {
  try {
    const response = await axios.get(`http://localhost:8080/kich-co/detail/${id}`, {
      headers: { Authorization: `Bearer ${token}` }
    });
...
<template>
  <div class="full-width-container">
    ...
    <div class="d-flex align-items-center justify-content-between bg-white p-3 rounded mb-3 border">
      <h2 class="fw-bold mb-0">Quản lý kích cỡ</h2>
      <div class="d-flex gap-2">
        <button
          ...
          class="btn"
        >
          <Plus class="me-1" size="16" /> Thêm kích cỡ
        </button>
      </div>
    </div>
    ...
    <div class="bg-white p-3 rounded mb-3 border">
      ...
      <input
        type="text"
        class="form-control"
        placeholder="Tìm theo mã, tên kích cỡ..."
        v-model="searchText"
      />
    </div>
    ...
    <div class="bg-white p-3 rounded mb-4 border">
      <h5 class="text-start ps-3 pt-3">Danh sách kích cỡ</h5>
