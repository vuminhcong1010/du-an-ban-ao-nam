...
const updateSearchResult = () => {
  const filtered = res.value.filter(item =>
    (item.maCoAo || "").toLowerCase().includes(searchText.value.toLowerCase()) ||
    (item.tenCoAo || "").toLowerCase().includes(searchText.value.toLowerCase())
  );
...
const update = async () => {
  ...
  const isDuplicate = res.value.some(
    (item) => item.tenCoAo.trim().toLowerCase() === req.value.tenCoAo.trim().toLowerCase() && item.id !== req.value.id
  );
  if (isDuplicate) {
    toast.error("Tên cổ áo đã tồn tại!");
    return;
  ...
  try {
    await axios.post("http://localhost:8080/co-ao/update", req.value, {
      headers: { Authorization: `Bearer ${token}` }
    });
...
const detail = async (id) => {
  try {
    const response = await axios.get(`http://localhost:8080/co-ao/detail/${id}`, {
      headers: { Authorization: `Bearer ${token}` }
    });
...
<template>
    ...
    <!-- Tiêu đề trang -->
    <div
      class="d-flex align-items-center justify-content-between bg-white p-3 rounded mb-3 border"
    >
      <h2 class="fw-bold mb-0">Quản lý cổ áo</h2>
      <div class="d-flex gap-2">
        <button
          ...
          class="btn"
        >
          <Plus class="me-1" size="16" /> Thêm cổ áo
        </button>
      </div>
    </div>
    ...
    <div class="bg-white p-3 rounded mb-4 border">
      <h5 class="text-start ps-3 pt-3">Danh sách cổ áo</h5>
