...
const add = () =>{
    axios.post("http://localhost:8080/mau/add",req.value).then(Response =>{
      res.value = Response.data.data
      page.value = Math.ceil(Response.data.size/10)
      reset()
    }).catch (err =>{
...
const update = () =>{
    ...
    axios.post("http://localhost:8080/mau/update",req.value).then(Response =>{
     res.value = Response.data.data
      page.value = Math.ceil(Response.data.size/10)
      reset()
    }).catch (err =>{
...
const remove = (id) =>{
    axios.get(`http://localhost:8080/mau/delete/${id}`).then(Response =>{
     res.value = Response.data.data
      page.value = Math.ceil(Response.data.size/10)
    }).catch (err =>{
       console.log(err);
...
const detail = (id) =>{
    ...
    axios.get(`http://localhost:8080/mau/detail/${id}`).then(Response =>{
...
const paging = (id) =>{
    axios.get(`http://localhost:8080/mau/${id}`).then(Response =>{
    res.value = Response.data.data
    page.value = Math.ceil(Response.data.size/10);
...
<template>
  <div class="container my-4">
    ...
    <div class="bg-white p-3 rounded shadow mb-4">
      ...
      <div class="bg-white p-3 rounded shadow mb-4">
      <div class="d-flex justify-content-between align-items-center mb-3">
        <h5 class="fw-semibold m-0">Quản lý màu</h5>
...
<div class="modal fade" id="exampleModal2" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="exampleModalLabel">Cập nhật thông tin Màu</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <div class="mb-3">
...
<div class="modal fade" id="exampleModal1" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="exampleModalLabel">Thông tin Màu</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <div class="mb-3">
        ...
        <label for="form1" class="form-label">Tên Màu</label>
        <input type="text" class="form-control" id="form1" v-model="details.ten" readonly>
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Đóng</button>
